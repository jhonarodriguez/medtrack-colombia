/**
 * Busca todas las sedes de Droguería Colsubsidio en Bogotá usando Google Maps Places API (New).
 * Uso: GOOGLE_MAPS_KEY=<key> npx tsx scripts/fetch-colsubsidio.ts
 */
...
async function main() {
  ...
  const queries = [
    "Droguería Colsubsidio Bogotá",
    "Drogería Colsubsidio Bogotá",
  ];
  ...
  for (const query of queries) {
    ...
    do {
      ...
      for (const p of data.places ?? []) {
        ...
        if (!/colsubsidio/i.test(p.displayName?.text ?? "")) continue;

        seenIds.add(p.id);
        // Limpiar dirección: quitar ", Bogotá, Cundinamarca, Colombia" etc.
...
`;
  ...
  writeFileSync("./src/adapters/sedes/colsubsidio-bogota.ts", ts, "utf-8");
  console.log("→ Guardado: src/adapters/sedes/colsubsidio-bogota.ts");
}
